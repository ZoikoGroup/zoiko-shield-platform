...
async function main() {
  ...
  console.log(
    ' 🛡️  ZoikoShield JIT (Just-In-Time) Dual-Authorized Elevation Simulator',
  );
  console.log(
    '    Specification: Dual-Authorization, Time-Bound Scoped Access & Customer Audit',
  );
  console.log(
    '========================================================================\n',
  );
  ...
  const request = await jitService.requestElevation({
    ...
    statedPurpose:
      'Diagnose P0 data exfiltration telemetry pipeline latency for Case #INC-9821',
  ...
  console.log(
    `  ✔ Requested Window: ${request.requestedDurationMinutes} minutes`,
  );
  ...
  console.log(
    `  ✔ Elevation Valid Until: ${approvedRequest.expiresAt?.toISOString()}`,
  );
  console.log(
    `  ✔ Customer Audit Ref: ${approvedRequest.customerVisibleAuditLogRef}`,
  );
  ...
  console.log(
    `  ✔ Time-Bound Expiration: ${activeMembership?.expiresAt?.toISOString()}`,
  );
  console.log(`  ✔ Elevation Purpose: "${activeMembership?.elevationPurpose}"`);
  console.log(
    `  ✔ Session Switching Status: READY (Real membership active in session context)`,
  );
  ...
  console.log(
    `  ✔ Sweeper executed -> Expired Memberships Count: ${sweepResult.expiredCount}`,
  );
  console.log(
    `  ✔ Post-Expiry Membership Status: ${activeMembership?.status} (Access revoked)`,
  );
  ...
  console.log(
    `  ✔ Found ${customerAuditTrail.length} JIT Elevation Record(s) for Customer '${tenantId}':`,
  );
  for (const entry of customerAuditTrail) {
    console.log(
      `    • [${entry.status}] Requester: ${entry.superAdminPrincipalId} | Approver: ${entry.approvedByPrincipalId} | Purpose: "${entry.statedPurpose}" (Audit Ref: ${entry.customerVisibleAuditLogRef})`,
    );
  }

  console.log(
    `  ✔ Customer Audit Events Recorded in Ledger: ${identityEvents.length} events`,
  );
  for (const ev of identityEvents) {
    console.log(
      `    ➔ Event: ${ev.eventType} on Tenant '${ev.tenantId}' (Actor: ${ev.actorId})`,
    );
  }

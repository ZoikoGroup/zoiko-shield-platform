...
async function main() {
  ...
  console.log(
    '    Specification: ZS-AI-SEC-001 §6 (Membership Inference Protection)',
  );
  console.log(
    '========================================================================\n',
  );
  ...
  console.log(
    `[1/3] Executing Privacy-Preserving Security Telemetry Aggregation...`,
  );
  console.log(
    `  ➔ Initial Tenant Epsilon Budget: ${dpGuard.getRemainingBudget(tenantId).toFixed(2)} ε`,
  );
  ...
  console.log(
    `  ✔ Sanitized Perturbed Value (Laplace Noise): ${query1.perturbedValue}`,
  );
  console.log(`  ✔ Calibrated Noise Added: ${query1.noiseAdded.toFixed(4)}`);
  console.log(
    `  ✔ Remaining Epsilon Budget: ${query1.remainingEpsilonBudget} ε`,
  );
  console.log(
    `  🔒 Differential Privacy Proof: ${query1.privacyProofDigest.slice(0, 32)}...`,
  );
  ...
  for (let i = 2; i <= 5; i++) {
    ...
    console.log(
      `  ✔ Query #${i} -> Perturbed: ${res.perturbedValue} | Epsilon Left: ${res.remainingEpsilonBudget} ε`,
    );
  }

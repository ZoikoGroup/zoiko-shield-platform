...
async function main() {
  ...
  console.log(
    ' 🛡️  ZoikoShield Cryptographic Shredding & Privacy Erasure Simulator',
  );
  console.log(
    '    Specification: ZS-DISP-SHRED-001 (GDPR Art. 17 / HIPAA Forget)',
  );
  console.log(
    '========================================================================\n',
  );
  ...
  const sensitivePii =
    'Medical Record: Patient John Smith, DOB: 1985-04-12, SSN: 999-12-8888, Diagnostic: Cardiac Arrhythmia';

  console.log(
    '[1/3] Ingesting Subject Evidence with Dedicated Subject Encryption Key (SEK)...',
  );
  const encryptedPayload = await shredder.encryptSubjectPii(
    tenantId,
    subjectId,
    sensitivePii,
  );
  ...
  console.log(
    `  ✔ Ciphertext (Stored in Merkle block): ${encryptedPayload.ciphertextHex.slice(0, 48)}...`,
  );
  ...
  const decryptedText = await shredder.decryptSubjectPii(encryptedPayload);
  console.log(`  ✔ Decrypted Content: "${decryptedText}"`);

  console.log(
    '\n[3/3] Executing GDPR Article 17 Right-to-be-Forgotten Cryptographic Erasure...',
  );
  const cert = await shredder.shredSubjectKey(tenantId, subjectId);

  console.log(`  ✔ Certificate ID: ${cert.certificateId}`);
  console.log(`  ✔ Erasure Method: ${cert.erasureType}`);
  console.log(`  ✔ Shredded Timestamp: ${cert.shreddedAt}`);
  console.log(
    `  ✔ Merkle Hash Chain Integrity Preserved: ${cert.merkleIntegrityPreserved}`,
  );
  console.log(
    `  🔒 Proof of Obliteration Digest: ${cert.proofOfObliterationDigest}`,
  );
  ...
  try {
    await shredder.decryptSubjectPii(encryptedPayload);
  } catch (err: any) {

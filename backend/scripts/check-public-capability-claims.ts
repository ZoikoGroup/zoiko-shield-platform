...
import { APPROVED_PLAN_TIERS } from '../apps/shield-core/src/modules/commercial/plan-tier/plan-tier.entity';
...
function verifyPublicCapabilityClaims(): void {
  ...
  console.log(`[x] Auditing ${publicServices.length} Customer-Visible Public Services...`);
  if (publicServices.length !== 12) {
    errors.push(`Expected exactly 12 public services, found ${publicServices.length}`);
  }
  ...
  for (const service of publicServices) {
    ...
    if (service.substantiatingComponents.length === 0) {
      errors.push(`Public service ${service.serviceId} (${service.serviceName}) has no substantiating components`);
    }
  }
  ...
  const isoActive = capabilityService.isFrameworkEvaluatorActive('ISO27001_A9_2');

  if (doraActive) errors.push('Rule FRM-01 Violation: DORA evaluator is marked active (must be DEFERRED per ADR-08)');
  if (nis2Active) errors.push('Rule FRM-01 Violation: NIS2 evaluator is marked active (must be DEFERRED per ADR-08)');
  if (pciActive) errors.push('Rule FRM-01 Violation: PCI DSS v4 evaluator is marked active (must be DEFERRED per ADR-08)');
  if (!soc2Active) errors.push('Rule FRM-01 Violation: SOC 2 CC6.1 evaluator must be active');
  if (!isoActive) errors.push('Rule FRM-01 Violation: ISO 27001 A.9.2 evaluator must be active');
  ...
  const sapGated = capabilityService.isConnectorAvailable('sap-enterprise-gated');

  if (!entraAvailable) errors.push('Tier 1 Microsoft Entra connector must be available');
  if (!awsAvailable) errors.push('Tier 1 AWS CloudTrail connector must be available');
  if (sapGated) errors.push('Rule CON-01 Violation: Gated Tier 3 connector exposed as available');

  // 4. Audit Plan Tiers
  console.log(`[x] Auditing ${APPROVED_PLAN_TIERS.length} Approved Commercial Plan Tiers...`);
  if (APPROVED_PLAN_TIERS.length !== 4) {
    errors.push(`Expected 4 plan tiers, found ${APPROVED_PLAN_TIERS.length}`);
  }

  const expectedTiers = ['SHIELD_ESSENTIAL', 'SHIELD_PROFESSIONAL', 'SHIELD_ADVANCED', 'SHIELD_ENTERPRISE'];
  for (const expected of expectedTiers) {
    const tier = APPROVED_PLAN_TIERS.find((t) => t.key === expected);
    if (!tier) {
      errors.push(`Missing approved plan tier: ${expected}`);
    } else {
      if (tier.key !== 'SHIELD_ENTERPRISE' && (!tier.pricing.monthlyUsd || tier.pricing.monthlyUsd <= 0)) {
        errors.push(`Plan tier ${tier.key} has invalid pricing: ${tier.pricing.monthlyUsd}`);
      }
    }
  }

  // 5. Audit Swagger / OpenAPI Documentation File
  console.log('[x] Auditing docs/swagger.yaml for deferred framework disclosures...');
  const swaggerPath = path.join(__dirname, '..', '..', 'docs', 'swagger.yaml');
  if (fs.existsSync(swaggerPath)) {
    const specRaw = fs.readFileSync(swaggerPath, 'utf8');
    const spec = yaml.load(specRaw) as any;
    const deferredInSpec = specRaw.includes('DEFERRED_PHASE2_MIDPOINT') || specRaw.includes('ADR-08');
    if (!deferredInSpec) {
      console.warn('Note: docs/swagger.yaml does not explicitly mention ADR-08 in description.');
    }
  }
  ...
  console.log('----------------------------------------------------------------');
  if (errors.length > 0) {
    console.error(`FAILED with ${errors.length} public capability violation(s):`);
    for (const err of errors) console.error(`  [!] ${err}`);
    process.exit(1);
  } else {
    console.log('SUCCESS: All 12 public services, 7 capability domains, connector tiers,');
    console.log('and commercial plan models strictly comply with GTM governance rules.');
    console.log('================================================================\n');
  }
}

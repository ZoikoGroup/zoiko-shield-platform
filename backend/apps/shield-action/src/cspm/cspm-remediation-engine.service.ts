...
export class CspmRemediationEngineService {
   ...
   */
  remediateMisconfiguration(finding: CspmDriftFinding): CspmSelfHealingExecutionReceipt {
    ...
    switch (finding.violationType) {
      case 'AWS_S3_PUBLIC_READ_WRITE':
        ...
        rollbackStateSnapshot = { BlockPublicAcls: false, IgnorePublicAcls: false };
        break;
      ...
      case 'AWS_SECURITY_GROUP_OPEN_INGRESS':
        ...
        rollbackStateSnapshot = { previousCidrIp: '0.0.0.0/0', ports: [22, 3389, 5432] };
        break;
    ...
    const attestationDigest = crypto
      .createHash('sha256')
      .update(JSON.stringify({ receiptId, finding, remediationAction, remediatedAt }))
      .digest('hex');

    this.logger.warn(`🛡️ [CSPM AUTO-REMEDIATION] Applied ${remediationAction} to ${finding.resourceArn}`);

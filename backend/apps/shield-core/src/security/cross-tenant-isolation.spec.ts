...
describe('Security — Cross-Tenant Isolation', () => {
  ...
  describe('Alert service tenant isolation', () => {
      ...
      prismaMock = {
        alert: {
          findMany: jest.fn().mockImplementation(({ where }) => {
            ...
            if (where?.tenant_id === TENANT_A) return [{ id: 'alert-a', tenant_id: TENANT_A }];
            return [];
          }),
    ...
    it('should only return alerts belonging to tenantA when queried as tenantA', async () => {
      const results = await prismaMock.alert.findMany({ where: { tenant_id: TENANT_A } });
      expect(results).toHaveLength(1);
      expect(results[0].tenant_id).toBe(TENANT_A);
    });

    it('should return ZERO alerts for tenantB when tenantA owns them', async () => {
      const results = await prismaMock.alert.findMany({ where: { tenant_id: TENANT_B } });
      expect(results).toHaveLength(0);
    });

    it('should never return tenantA alerts when requesting tenantB', async () => {
      const results = await prismaMock.alert.findMany({ where: { tenant_id: TENANT_B } });
      const crossTenantLeak = results.some((a: any) => a.tenant_id === TENANT_A);
  ...
  describe('Case service tenant isolation', () => {
    ...
    beforeEach(() => {
      prismaMock = {
        case: {
          findMany: jest.fn().mockImplementation(({ where }) => {
            if (where?.tenant_id === TENANT_A) return [{ id: 'case-a', tenant_id: TENANT_A }];
            return [];
          }),
        },
      };
    ...
    it('should return cases only for tenantA', async () => {
      const results = await prismaMock.case.findMany({ where: { tenant_id: TENANT_A } });
      expect(results.every((c: any) => c.tenant_id === TENANT_A)).toBe(true);
    });

    it('should not expose tenantA cases to tenantB', async () => {
      const results = await prismaMock.case.findMany({ where: { tenant_id: TENANT_B } });
      expect(results).toHaveLength(0);
    });
  });
  ...
  describe('Evidence record tenant isolation', () => {
    ...
    beforeEach(() => {
      prismaMock = {
        evidenceRecord: {
          findMany: jest.fn().mockImplementation(({ where }) => {
            if (where?.tenant_id === TENANT_A) return [{ id: 'ev-a', tenant_id: TENANT_A }];
            return [];
          }),
        },
      };
    ...
    it('should return evidence only for tenantA', async () => {
      const results = await prismaMock.evidenceRecord.findMany({ where: { tenant_id: TENANT_A } });
      expect(results.every((e: any) => e.tenant_id === TENANT_A)).toBe(true);
    });

    it('should not expose tenantA evidence to tenantB', async () => {
      const results = await prismaMock.evidenceRecord.findMany({ where: { tenant_id: TENANT_B } });
      expect(results).toHaveLength(0);
    });
  });
  ...
  describe('Audit package tenant isolation', () => {
    ...
    it('should find tenantA package when queried by tenantA', async () => {
      const pkg = await prismaMock.auditPackage.findFirst({ where: { tenant_id: TENANT_A, id: 'pkg-a' } });
      expect(pkg).not.toBeNull();
      expect(pkg.tenant_id).toBe(TENANT_A);
    });

    it('should NOT find tenantA package when queried by tenantB', async () => {
      const pkg = await prismaMock.auditPackage.findFirst({ where: { tenant_id: TENANT_B, id: 'pkg-a' } });
      expect(pkg).toBeNull();
    });
  });
});

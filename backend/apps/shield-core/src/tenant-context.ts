...
/** Resolve the one canonical tenant identifier accepted by a request. */
...
export function requireEnvironmentId(
  ...
  if (environmentIds.length === 0 || environmentIds.includes('default-env')) {
    throw new BadRequestException(
      'A canonical environment identifier is required',
    );
  }
  if (environmentIds.length > 1) {
    throw new BadRequestException(
      'Conflicting environment identifiers were supplied',
    );
  }
...
export function requireRegion(
  ...
  if (regions.length === 0 || regions.includes('unspecified')) {
    throw new BadRequestException(
      'A canonical data-residency region is required',
    );
  }
  if (regions.length > 1) {
    throw new BadRequestException(
      'Conflicting data-residency regions were supplied',
    );
  }
  return regions[0];
}

...
describe('CustomerDisclosuresController', () => {
  ...
  beforeEach(async () => {
    ...
    controller = module.get<CustomerDisclosuresController>(CustomerDisclosuresController);
  });
  ...
  it('returns valid customer disclosures declaring G1 pending status, R1 authority, and ADR-08 deferral', () => {
    ...
    expect(disclosures.g1LaunchGateStatus).toBe('PENDING MULTI-APPROVER SIGN-OFF');
    expect(disclosures.responseAuthorityLevel).toBe('R1_RECOMMEND_AND_SIMULATE_ONLY');
    ...
    expect(disclosures.certifiedConnectorsScope.p0Certified).toContain('Microsoft Entra ID / M365');

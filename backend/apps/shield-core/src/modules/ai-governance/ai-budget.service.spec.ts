...
describe('AiBudgetService (fail closed on missing budget)', () => {
  ...
  beforeEach(async () => {
    prismaMock = { aiBudget: { create: jest.fn(), findFirst: jest.fn(), update: jest.fn() } };

    const module: TestingModule = await Test.createTestingModule({
      providers: [AiBudgetService, { provide: PrismaService, useValue: prismaMock }],
    }).compile();
  ...
  it('is not over budget while consumed is below the configured amount', async () => {
    prismaMock.aiBudget.findFirst.mockResolvedValue({ budget_amount: 1000, consumed_amount: 200 });
  ...
  it('flips to EXHAUSTED once spend reaches the budget amount', async () => {
    prismaMock.aiBudget.findFirst.mockResolvedValue({ id: 'b-1', budget_amount: 100, consumed_amount: 90 });
    prismaMock.aiBudget.update.mockResolvedValue({ id: 'b-1', status: 'EXHAUSTED' });

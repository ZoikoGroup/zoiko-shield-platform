...
export class EvaluationReplayService {
  ...
  async replay(evaluationRunId: string) {
    const original = await this.prisma.evaluationRun.findUnique({
      where: { id: evaluationRunId },
    });
    if (!original) {
      throw new NotFoundException(
        `EvaluationRun '${evaluationRunId}' not found`,
      );
    }
    if (!original.replayable) {
      throw new NotFoundException(
        `EvaluationRun '${evaluationRunId}' is not marked replayable`,
      );
    }

    const bundle = await this.evidenceBundleService.getById(
      original.evidence_bundle_id,
    );
    const evaluatorVersion =
      await this.evaluatorRegistry.getVersionWithEvaluator(
        original.evaluator_version_id,
      );
      ...
      records.map(async (r) => {
        ...
        if (r.vault_reference) {
          try {
            ...
            this.logger.warn(
              `Failed to load evidence content for ${r.id} during replay: ${(err as Error).message}`,
            );
          }
        }
        return {
    ...
    try {
      ...
      outcome = {
        result: 'ERROR',
        rationale: `Evaluator threw during replay: ${(err as Error).message}`,
    ...
    const finalResult =
      inputBundleHash === original.input_bundle_hash &&

...
export class WebhookSecretService {
  ...
  async issue(
  ...
  ): Promise<{ version: number; secretRef: string }> {
    const latest = await this.prisma.webhookSecretVersion.findFirst({
      where: { webhook_subscription_id: webhookSubscriptionId },
    ...
    if (latest) {
      await this.prisma.webhookSecretVersion.update({
        where: { id: latest.id },
    ...
    this.logger.debug(
      `Issued webhook secret version ${nextVersion} for subscription ${webhookSubscriptionId} (dev vault ref, not logged)`,
    );
  ...
  async getActiveSecret(webhookSubscriptionId: string): Promise<string | null> {
    const active = await this.prisma.webhookSecretVersion.findFirst({
      where: {
        webhook_subscription_id: webhookSubscriptionId,
        status: 'ACTIVE',
      },

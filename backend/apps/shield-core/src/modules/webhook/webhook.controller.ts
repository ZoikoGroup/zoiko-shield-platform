...
export class WebhookController {
  ...
  async create(
    ...
    @Body()
    body: { endpointUrl: string; eventTypes: string[]; apiClientId?: string },
  ) {
  ...
  async list(@Headers('x-tenant-id') tenantId: string) {
    return this.prisma.outboundWebhookSubscription.findMany({
  ...
  async getById(
    ...
    return this.subscriptionService.assertTenantOwnership(
  ...
  async patch(
    ...
    if (body.status === 'SUSPENDED')
      return this.subscriptionService.suspend(requireTenantId(tenantId), id);
    return this.subscriptionService.assertTenantOwnership(
      requireTenantId(tenantId),
      id,
    );
  ...
  async getDelivery(
    ...
    return this.prisma.webhookDelivery.findFirst({
      where: { id: deliveryId, tenant_id: requireTenantId(tenantId) },

...
export class TenantOffboardingService {
  ...
  async start(tenantId: string, requestedBy: string, reason: string) {
    ...
    if (decision === 'DENY') {
      throw new ForbiddenException('Actor is not authorized to start tenant offboarding');
    }

    const existing = await this.prisma.tenantOffboardingRun.findFirst({ where: { tenant_id: tenantId, status: { notIn: ['COMPLETED', 'FAILED', 'BLOCKED'] } } });
    ...
    const [run] = await this.prisma.$transaction([
      this.prisma.tenantOffboardingRun.create({
        data: { id: randomUUID(), tenant_id: tenantId, requested_by: requestedBy, reason, status: 'EXPORT_REQUIRED', authorization_decision_id: authorizationDecisionId },
      }),
      this.prisma.outboxEvent.create({ data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.TENANT_OFFBOARDING_STARTED, eventType: 'tenant.offboarding.started', payload: {} }) }),
    ]);
    return run;
  }
  ...
  async startFinalExport(tenantId: string, runId: string) {
    ...
    const job = await this.exportJobService.create({
      tenantId, requestedBy: run.requested_by, purpose: 'TENANT_OFFBOARDING_FINAL_EXPORT', exportType: 'FULL_TENANT',
      requestedScope: ['cases', 'alerts', 'controls', 'assessments', 'risks', 'exceptions', 'evidence_metadata', 'audit_packages'],
    });
    await this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { status: 'EXPORTING', export_job_id: job.id } });
    await this.exportWorkerService.run(job.id);

    const finished = await this.prisma.exportJob.findUniqueOrThrow({ where: { id: job.id } });
    if (finished.status !== 'READY' && finished.status !== 'PARTIAL') {
      ...
      throw new ConflictException(`Final export did not complete (status ${finished.status}) — offboarding cannot proceed until it does`);
    }
    return this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { status: 'EXPORT_READY' } });
  }

  async freezeAccess(tenantId: string, runId: string) {
    const run = await this.assertOwnership(tenantId, runId);
    if (run.status !== 'EXPORT_READY') {
      throw new ConflictException(`Offboarding run '${runId}' must be EXPORT_READY before freezing access (currently ${run.status})`);
    }
    const [updated] = await this.prisma.$transaction([
      this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { status: 'ACCESS_FROZEN' } }),
      this.prisma.outboxEvent.create({ data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.TENANT_ACCESS_FROZEN, eventType: 'tenant.access.frozen', payload: {} }) }),
    ]);

    // API clients: SUSPENDED, then credentials REVOKED (spec §76) — no new token issuance from here on.
    const apiClients = await this.prisma.apiClient.findMany({ where: { tenant_id: tenantId, status: 'ACTIVE' } });
  ...
  async revokeConnectors(tenantId: string, runId: string) {
    ...
    if (run.status !== 'ACCESS_FROZEN') {
      throw new ConflictException(`Offboarding run '${runId}' must be ACCESS_FROZEN before revoking connectors (currently ${run.status})`);
    }
    await this.prisma.connectorInstance.updateMany({ where: { tenant_id: tenantId }, data: { state: 'NOT_CONNECTED' } });
    const [updated] = await this.prisma.$transaction([
      this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { status: 'CONNECTOR_REVOCATION' } }),
      this.prisma.outboxEvent.create({ data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.TENANT_CONNECTORS_REVOKED, eventType: 'tenant.connectors.revoked', payload: {} }) }),
    ]);
    return updated;
  }
  ...
  async startDeletion(tenantId: string, runId: string, requestedBy: string) {
    ...
    if (run.status !== 'CONNECTOR_REVOCATION') {
      throw new ConflictException(`Offboarding run '${runId}' must have completed connector revocation before deletion (currently ${run.status})`);
    }

    const deletionRequest = await this.deletionRequestService.request({ tenantId, requestedBy, reason: 'Tenant offboarding', scope: { all: true } });
    await this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { deletion_request_id: deletionRequest.id, status: deletionRequest.status === 'BLOCKED_BY_HOLD' ? 'BLOCKED' : 'DELETION_PENDING' } });

    if (deletionRequest.status === 'BLOCKED_BY_HOLD') {
      return this.prisma.tenantOffboardingRun.findUniqueOrThrow({ where: { id: run.id } });
    }

    await this.prisma.outboxEvent.create({ data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.TENANT_DELETION_STARTED, eventType: 'tenant.deletion.started', payload: { deletionRequestId: deletionRequest.id } }) });
    await this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { status: 'DELETING' } });
    ...
    await this.prisma.outboxEvent.create({ data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.TENANT_DELETION_RECONCILED, eventType: 'tenant.deletion.reconciled', payload: { deletionRequestId: deletionRequest.id } }) });
    await this.prisma.outboxEvent.create({ data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.TENANT_BACKUP_EXPIRY_PENDING, eventType: 'tenant.backup_expiry.pending', payload: { deletionRequestId: deletionRequest.id } }) });

    return this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { status: 'BACKUP_EXPIRY_PENDING' } });
  }

  async issueAttestationAndClose(tenantId: string, runId: string, issuedBy: string) {
    const run = await this.assertOwnership(tenantId, runId);
    if (run.status !== 'BACKUP_EXPIRY_PENDING' || !run.deletion_request_id) {
      throw new ConflictException(`Offboarding run '${runId}' is not ready for attestation (currently ${run.status})`);
    }
    ...
    const [updated] = await this.prisma.$transaction([
      this.prisma.tenantOffboardingRun.update({ where: { id: run.id }, data: { status: 'COMPLETED', completed_at: new Date() } }),
      this.prisma.outboxEvent.create({ data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.TENANT_CLOSED, eventType: 'tenant.closed', payload: {} }) }),
    ]);
    return { run: updated, attestation };
  }

  async assertOwnership(tenantId: string, runId: string) {
    const run = await this.prisma.tenantOffboardingRun.findFirst({ where: { id: runId, tenant_id: tenantId } });
    if (!run) {

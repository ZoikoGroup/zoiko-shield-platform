...
const NO_STORE_POPULATED = JSON.stringify({ outcome: 'NOT_APPLICABLE', reason: 'STORE_NOT_CONFIGURED_OR_POPULATED' });
...
export class DeletionTaskService {
      ...
      switch (task.store_type) {
        case 'POSTGRES_AUTHORITY':
          verificationResult = await this.deleteAuthoritativeRows(
            task.tenant_id,
            (await this.prisma.deletionRequest.findUniqueOrThrow({ where: { id: task.deletion_request_id } })).requested_by,
          );
          break;
        case 'OBJECT_STORAGE':
  ...
  private async deleteAuthoritativeRows(tenantId: string, closingOperatorId: string): Promise<string> {
    ...
    `;
    const candidates = tables.map((row) => row.table_name).filter((name) => !DELETION_CONTROL_TABLES.has(name));
    ...
    while (remaining.size > 0) {
      const leaves = [...remaining].filter((parent) => !foreignKeys.some((fk) =>
        fk.parent_table === parent && remaining.has(fk.child_table) && fk.child_table !== parent,
      ));
      if (leaves.length === 0) {
        throw new Error(`Tenant deletion dependency cycle detected among: ${[...remaining].sort().join(', ')}`);
      }
    ...
    await this.prisma.$transaction(async (tx) => {
      for (const table of ordered) {
        if (!candidateSet.has(table) || !/^[A-Za-z_][A-Za-z0-9_]*$/.test(table)) {
          throw new Error(`Unsafe tenant table identifier '${table}'`);
        }
        counts[table] = await tx.$executeRawUnsafe(`DELETE FROM "${table}" WHERE tenant_id = $1`, tenantId);
      }
      ...
      counts['authorization.user_roles'] = await tx.$executeRawUnsafe(
        'DELETE FROM authorization.user_roles WHERE membership_id IN (SELECT id FROM authorization.tenant_memberships WHERE "tenantId" = $1::uuid AND "principalId" <> $2::uuid)', tenantId, closingOperatorId,
      );
      counts['authorization.tenant_memberships'] = await tx.$executeRawUnsafe(
        'DELETE FROM authorization.tenant_memberships WHERE "tenantId" = $1::uuid AND "principalId" <> $2::uuid', tenantId, closingOperatorId,
      );
      counts['authorization.invitations'] = await tx.$executeRawUnsafe(
        'DELETE FROM authorization.invitations WHERE "tenantId" = $1', tenantId,
      );
      counts['authorization.role_permissions'] = await tx.$executeRawUnsafe(
        'DELETE FROM authorization.role_permissions WHERE role_id IN (SELECT role.id FROM authorization.roles role WHERE role."tenantId" = $1::uuid AND NOT EXISTS (SELECT 1 FROM authorization.user_roles user_role WHERE user_role.role_id = role.id))', tenantId,
      );
      counts['authorization.roles'] = await tx.$executeRawUnsafe(
        'DELETE FROM authorization.roles role WHERE role."tenantId" = $1::uuid AND NOT EXISTS (SELECT 1 FROM authorization.user_roles user_role WHERE user_role.role_id = role.id)', tenantId,
      );
      counts['identity.identity_events'] = await tx.$executeRawUnsafe(
        'DELETE FROM identity.identity_events WHERE "tenantId" = $1::uuid', tenantId,
      );
      for (const table of ['customers', 'organizations', 'environments', 'legal_entities']) {
        counts[`tenant.${table}`] = await tx.$executeRawUnsafe(
          `DELETE FROM tenant.${table} WHERE "tenantId" = $1::uuid`, tenantId,
        );
      }
      counts['tenant.tenants'] = await tx.$executeRawUnsafe('DELETE FROM tenant.tenants WHERE id = $1::uuid', tenantId);
    });

    let remainingRows = 0;
    for (const table of ordered) {
      const rows = await this.prisma.$queryRawUnsafe<Array<{ count: bigint }>>(
        `SELECT COUNT(*)::bigint AS count FROM "${table}" WHERE tenant_id = $1`, tenantId,
      );
      remainingRows += Number(rows[0]?.count ?? 0);
    }
    const typeOrmRemaining = await this.prisma.$queryRawUnsafe<Array<{ count: bigint }>>(`
      SELECT (
    ...
    `, tenantId, closingOperatorId);
    remainingRows += Number(typeOrmRemaining[0]?.count ?? 0);
    if (remainingRows !== 0) throw new Error(`${remainingRows} tenant-scoped PostgreSQL row(s) remain after deletion`);
  ...
  private async deleteObjectStorage(tenantId: string): Promise<string> {
    ...
    if (result.remaining !== 0) throw new Error(`${result.remaining} tenant object(s) remain after object-storage deletion`);
    return JSON.stringify({ outcome: 'VERIFIED_DELETED', deletedObjects: result.deleted, remainingObjects: 0 });
  }
  ...
  private async revokeConnectorState(tenantId: string): Promise<string> {
    ...
    const remaining = await this.prisma.connectorInstance.count({ where: { tenant_id: tenantId, deletedAt: null } });
    if (remaining !== 0) throw new Error(`${remaining} active connector instance(s) remain after revocation`);
    return JSON.stringify({ outcome: 'VERIFIED_DELETED', revokedConnectors: result.count, remainingActiveConnectors: 0 });
  }
}

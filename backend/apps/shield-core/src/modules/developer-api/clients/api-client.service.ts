...
export class ApiClientService {
    ...
    const [apiClient] = await this.prisma.$transaction([
      ...
      this.prisma.outboxEvent.create({
        data: this.outbox.build({
          ...
          topic: CANONICAL_TOPICS.API_CLIENT_CREATED,
  ...
  async verifyCredential(
    clientId: string,
    rawSecret: string,
  ): Promise<{ apiClient: any; tenantId: string } | null> {
    ...
    if (
      !credential ||
      (credential.expires_at && credential.expires_at < new Date())
    )
      return null;
  ...
  async suspend(tenantId: string, apiClientId: string) {
    ...
    const [updated] = await this.prisma.$transaction([
      ...
      this.prisma.outboxEvent.create({
        data: this.outbox.build({
          tenantId,
          topic: CANONICAL_TOPICS.API_CLIENT_SUSPENDED,
          eventType: 'api_client.suspended',
  ...
  async revoke(tenantId: string, apiClientId: string) {
    ...
    const [updated] = await this.prisma.$transaction([
      ...
      this.prisma.apiClientCredential.updateMany({
        where: { api_client_id: client.id, status: { not: 'REVOKED' } },
        data: { status: 'REVOKED', revoked_at: new Date() },
      }),
      this.prisma.outboxEvent.create({
        data: this.outbox.build({
          tenantId,
          topic: CANONICAL_TOPICS.API_CLIENT_REVOKED,
  ...
  async assertTenantOwnership(tenantId: string, apiClientId: string) {
    const client = await this.prisma.apiClient.findFirst({
      where: { id: apiClientId, tenant_id: tenantId },
    });
    if (!client) {

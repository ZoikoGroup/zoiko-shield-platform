...
import { AlertStateMachineService, AlertStatus } from '../state-machine/alert-state-machine.service';
...
export class AlertService {
  ...
  private async transition(
    ...
    const [updated] = await this.prisma.$transaction([
      this.prisma.alert.update({ where: { id: alertId }, data: { status: toState, ...extraData } }),
  ...
  async acknowledge(tenantId: string, alertId: string) {
    return this.transition(tenantId, alertId, 'ACKNOWLEDGED', { acknowledged_at: new Date() }, ALERT_TOPICS.ALERT_ACKNOWLEDGED, 'alert.acknowledged');
  }

  async triage(tenantId: string, alertId: string) {
    return this.transition(tenantId, alertId, 'TRIAGED', {}, ALERT_TOPICS.ALERT_TRIAGED, 'alert.triaged');
  }
  ...
  async escalate(tenantId: string, alertId: string) {
    return this.transition(tenantId, alertId, 'ESCALATED_TO_CASE', {}, ALERT_TOPICS.ALERT_ESCALATED, 'alert.escalated');
  }
  ...
  async close(tenantId: string, alertId: string, disposition?: AlertStatus) {
    ...
    if (target !== 'CLOSED') {
      ...
      await this.transition(tenantId, alertId, target, {}, ALERT_TOPICS.ALERT_TRIAGED, `alert.${target.toLowerCase()}`);
    }
    return this.transition(tenantId, alertId, 'CLOSED', { resolved_at: new Date() }, ALERT_TOPICS.ALERT_TRIAGED, 'alert.closed');
  }

  async updateStatus(tenantId: string, alertId: string, status: string) {
    return this.transition(tenantId, alertId, status as AlertStatus, {}, ALERT_TOPICS.ALERT_TRIAGED, 'alert.status.changed');
  }
}

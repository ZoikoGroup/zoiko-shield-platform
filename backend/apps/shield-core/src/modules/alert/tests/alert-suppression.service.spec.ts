...
describe('AlertSuppressionService', () => {
  ...
  beforeEach(async () => {
    prismaMock = { alertSuppressionRule: { create: jest.fn(), findMany: jest.fn() } };
    const module: TestingModule = await Test.createTestingModule({
      providers: [AlertSuppressionService, { provide: PrismaService, useValue: prismaMock }],
    }).compile();
  ...
  it('matches an active, in-window, scope-matching suppression rule', async () => {
    prismaMock.alertSuppressionRule.findMany.mockResolvedValue([
      { id: 'rule-1', detection_definition_id: 'def-1', identity_id: null, asset_id: null, reason: 'known test account' },
    ]);

    const match = await service.findActiveMatch({ tenantId: 'tenant-a', detectionDefinitionId: 'def-1' });
  ...
  it('does not match a rule scoped to a different detection definition', async () => {
    prismaMock.alertSuppressionRule.findMany.mockResolvedValue([
      { id: 'rule-1', detection_definition_id: 'def-OTHER', identity_id: null, asset_id: null, reason: 'irrelevant' },
    ]);

...
export interface CreateDeficiencyInput {
  ...
  type: 'DESIGN_GAP' | 'IMPLEMENTATION_GAP' | 'OPERATING_FAILURE' | 'EVIDENCE_GAP' | 'COVERAGE_GAP';
  severity: string;
  description: string;
  evidenceRefs?: string[];
  ownerId: string;
...
export class ControlDeficiencyService {
  ...
  async createFromAssessment(assessment: { id: string; tenant_id: string; control_implementation_id: string; effectiveness: string; limitations: string }, controlObjectiveId: string, ownerId: string) {
    const type = assessment.effectiveness === 'INEFFECTIVE' ? 'OPERATING_FAILURE' : 'IMPLEMENTATION_GAP';
    return this.create({

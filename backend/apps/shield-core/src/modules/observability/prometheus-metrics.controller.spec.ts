...
describe('PrometheusMetricsController', () => {
  ...
  it('returns valid Prometheus text format metrics with tenant labels', () => {
    ...
    expect(output).toContain(
      'zoikoshield_ingest_acceptance_rate{tenant_id="tenant-corp-01",connector_state="HEALTHY"} 99.80',
    );
    expect(output).toContain(
      'zoikoshield_merkle_tree_leaves_total{tenant_id="tenant-corp-01"} 450',
    );
    expect(output).toContain(
      'zoikoshield_action_freeze_active{tenant_id="tenant-corp-01",scope="TENANT"} 0',
    );
  });
});

...
import { PhaseExitGateService, Phase0ExitProofRecord } from './phase-exit-gate.service';
...
export class PhaseProofExporterService {
  ...
  public exportPhase0ProofBundle(proofId?: string): Phase0ProofBundle {
    ...
    if (!proof) {
      throw new NotFoundException(`Phase-0 Exit Proof not found for ID: ${proofId}`);
    }

    const packageId = `pkg-phase0-${proof.proofId.replace('phase0-proof-', '')}`;
    const leaves = [
      { index: 0, leafHash: proof.steps[0].evidenceDigest, label: 'TENANT_PROVISIONING_LEAF' },
      { index: 1, leafHash: proof.steps[1].evidenceDigest, label: 'TELEMETRY_INGESTION_LEAF' },
      { index: 2, leafHash: proof.steps[2].evidenceDigest, label: 'DETERMINISTIC_DETECTION_LEAF' },
      { index: 3, leafHash: proof.steps[3].evidenceDigest, label: 'EVIDENCE_CONTROL_LEAF' },
      { index: 4, leafHash: proof.steps[4].evidenceDigest, label: 'AUDIT_PACKAGE_LEAF' },
      { index: 5, leafHash: proof.steps[5].evidenceDigest, label: 'WITNESS_ANCHOR_LEAF' },
      { index: 6, leafHash: proof.steps[6].evidenceDigest, label: 'ACTION_SIMULATION_LEAF' },
      { index: 7, leafHash: proof.steps[7].evidenceDigest, label: 'FREEZE_ASSERTION_LEAF' },
    ];
    ...
    const bundle: Phase0ProofBundle = {
      ...
      evidenceChain: {
        evidenceId: proof.steps[3].outputArtifacts.evidenceId || 'ev-rec-default',
        controlId: 'CTRL_ACCESS_GOVERNANCE_01',
        controlAssessment: 'SATISFIED',
        completenessRatio: 1.0,
        detectionAlertId: proof.steps[2].outputArtifacts.alertId || 'alt-det-default',
        ruleId: 'RULE_CANARY_PRIV_ESC_DETERMINISTIC_01',
      },
      actionSandboxReceipt: {
        simulationId: proof.steps[6].outputArtifacts.simulationId || 'sim-soar-default',
    ...
    this.logger.log(`[Spec §28] Generated Phase-0 proof bundle ${packageId} for tenant ${proof.targetTenantId}`);
    return bundle;
  }
   ...
   */
  public verifyProofBundleOffline(bundle: Phase0ProofBundle): OfflineVerificationReport {
    ...
    const merkleRootMatches = bundle.manifest.merkleRootHead === bundle.merkleTreeData.rootHash;
    if (!merkleRootMatches) {
      discrepancies.push('Manifest Merkle root does not match Merkle tree root hash');
    }

    // 3. Check evidence chain & 0 live mutations in simulation
    const liveMutationsZero = bundle.actionSandboxReceipt.liveMutationsCount === 0;
    if (!liveMutationsZero) {
      discrepancies.push('Action sandbox reported uncontained live mutations in Phase-0 simulation');
    }
    ...
    const signatureMatches = !!bundle.manifest.proofSignatureSha256 && bundle.manifest.proofSignatureSha256.length === 64;
    if (!signatureMatches) {
    ...
    const verified = discrepancies.length === 0 && invariantsPassed === invariants.length;

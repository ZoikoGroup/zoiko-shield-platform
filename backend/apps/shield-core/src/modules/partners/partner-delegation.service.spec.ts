...
describe('PartnerDelegationService (ZS-COM-BILL-001 Part 21: operational access != commercial authority)', () => {
  ...
  it('grants a delegation when every requested scope is on the allowlist', async () => {
    prismaMock.partnerDelegation.create.mockResolvedValue({
      id: 'del-1',
      status: 'ACTIVE',
    });
  ...
  it('checkDelegation is fail-closed (non-transitive): a delegation for a different commercial account never matches', async () => {
    ...
    prismaMock.partnerDelegation.findFirst.mockImplementation(
      ({ where }: any) =>
        Promise.resolve(
          where.commercial_account_id === 'acct-1'
            ? {
                id: 'del-1',
                scope: '["VIEW_USAGE"]',
                status: 'ACTIVE',
    ...
    const allowedForCorrectAccount = await service.checkDelegation(
      'p-1',
      'acct-1',
      'VIEW_USAGE',
    );
    const allowedForOtherAccount = await service.checkDelegation(
      'p-1',
      'acct-2',
      'VIEW_USAGE',
    );
  ...
  it('dynamically expires a delegation past its expires_at, before a sweeper runs, and denies the check', async () => {
    ...
    prismaMock.partnerDelegation.update.mockResolvedValue({
      id: 'del-1',
      status: 'EXPIRED',
    });
  ...
  it('rejects revoking a delegation that is not ACTIVE', async () => {
    prismaMock.partnerDelegation.findUnique.mockResolvedValue({
      id: 'del-1',
      status: 'REVOKED',
    });

...
export class PartnerDelegationService {
  ...
  async grantDelegation(dto: GrantDelegationDto) {
    const invalidScopes = dto.scope.filter((s) => !ALLOWED_DELEGATION_SCOPES.includes(s as any));
  ...
  async getDelegationById(id: string) {
    const delegation = await this.prisma.partnerDelegation.findUnique({ where: { id } });
    if (!delegation) {
  ...
  async revoke(id: string) {
    ...
    if (delegation.status !== 'ACTIVE') {
      throw new ConflictException(`Delegation '${id}' is '${delegation.status}', not ACTIVE`);
    }
   ...
   * sweeper runs.
   */
  async checkDelegation(partnerId: string, commercialAccountId: string, requiredScope: string): Promise<boolean> {
    const delegation = await this.prisma.partnerDelegation.findFirst({
      where: { partner_id: partnerId, commercial_account_id: commercialAccountId, status: 'ACTIVE' },
    ...
    if (delegation.expires_at && delegation.expires_at < new Date()) {
      await this.prisma.partnerDelegation.update({ where: { id: delegation.id }, data: { status: 'EXPIRED' } });
      return false;
    }

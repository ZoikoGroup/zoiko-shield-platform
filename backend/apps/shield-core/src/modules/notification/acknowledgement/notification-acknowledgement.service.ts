...
export class NotificationAcknowledgementService {
  ...
  async acknowledge(params: {
    ...
    const delivery = await this.prisma.notificationDelivery.findFirst({
      where: { id: params.notificationDeliveryId, tenant_id: params.tenantId },
    });
    if (!delivery) {
      throw new NotFoundException(
        `NotificationDelivery '${params.notificationDeliveryId}' not found`,
      );
    }
    if (delivery.recipient_principal_id !== params.principalId) {
      throw new ForbiddenException(
        'Only the recipient of a notification may acknowledge it',
      );
    }

...
function makeDeps(
  ...
  const auditPackageService = {
    assertTenantOwnership: jest.fn().mockResolvedValue(pkg),
  } as any;
...
const baseManifest = {
  ...
  manifest_core_content: JSON.stringify({
    evaluationIndex: [],
    assessmentIndex: [],
  }),
};
...
describe('AuditPackageApprovalService (segregation of duties)', () => {
  it('rejects an approver who is also the package creator', async () => {
    ...
    const service = new AuditPackageApprovalService(
      deps.prisma,
      deps.authorizationDecisionService,
      deps.outbox,
      deps.auditPackageService,
      deps.stateMachine,
    );
    await expect(service.approve('t1', 'pkg1', 'creator1')).rejects.toThrow(
      /creator/,
    );
  });

  it('rejects an approver who authored an evaluator whose results are included in the package', async () => {
    const manifestWithEvaluator = {
      ...baseManifest,
      manifest_core_content: JSON.stringify({
        evaluationIndex: [{ evaluatorVersionId: 'ev1' }],
        assessmentIndex: [],
      }),
    };
    const deps = makeDeps(basePkg, manifestWithEvaluator, [
      { id: 'ev1', evaluator: { owner: 'approver-x' } },
    ]);
    const service = new AuditPackageApprovalService(
      deps.prisma,
      deps.authorizationDecisionService,
    ...
    await expect(service.approve('t1', 'pkg1', 'approver-x')).rejects.toThrow(
      /authored an evaluator/,
    );
  });

  it('rejects an approver who performed or reviewed an included manual test', async () => {
    const manifestWithAssessment = {
      ...baseManifest,
      manifest_core_content: JSON.stringify({
        evaluationIndex: [],
        assessmentIndex: [{ assessmentId: 'a1' }],
      }),
    };
    const deps = makeDeps(
      basePkg,
      manifestWithAssessment,
      [],
      [{ performer_id: 'approver-y', reviewer_id: null }],
    );
    const service = new AuditPackageApprovalService(
      deps.prisma,
      deps.authorizationDecisionService,
    ...
    await expect(service.approve('t1', 'pkg1', 'approver-y')).rejects.toThrow(

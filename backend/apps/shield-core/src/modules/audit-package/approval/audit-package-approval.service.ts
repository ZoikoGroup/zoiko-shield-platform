...
export class AuditPackageApprovalService {
  ...
  async approve(tenantId: string, packageId: string, approverId: string) {
    ...
    if (approverId === pkg.created_by) {
      throw new ForbiddenException('Package approver must not be the package creator (segregation of duties)');
    }
    ...
    if (!manifest || !manifest.manifest_core_hash) {
      throw new NotFoundException(`AuditPackage '${packageId}' has no built manifest to approve`);
    }

    const manifestCore = JSON.parse(manifest.manifest_core_content);
    const evaluatorVersionIds: string[] = (manifestCore.evaluationIndex ?? []).map((e: { evaluatorVersionId: string }) => e.evaluatorVersionId).filter(Boolean);
    if (evaluatorVersionIds.length > 0) {
      const evaluatorVersions = await this.prisma.evaluatorVersion.findMany({ where: { id: { in: evaluatorVersionIds } }, include: { evaluator: true } });
      const authors = new Set(evaluatorVersions.map((v) => v.evaluator.owner));
      if (authors.has(approverId)) {
        throw new ForbiddenException('Package approver must not have authored an evaluator whose results are included in this package (segregation of duties)');
      }
    }

    const assessmentIds: string[] = (manifestCore.assessmentIndex ?? []).map((a: { assessmentId: string }) => a.assessmentId);
    if (assessmentIds.length > 0) {
      const manualRuns = await this.prisma.manualTestRun.findMany({ where: { tenant_id: tenantId } });
      const relevantPerformers = new Set(manualRuns.map((r) => r.performer_id));
      const relevantReviewers = new Set(manualRuns.filter((r) => r.reviewer_id).map((r) => r.reviewer_id as string));
      if (relevantPerformers.has(approverId) || relevantReviewers.has(approverId)) {
        throw new ForbiddenException('Package approver must not have performed or reviewed a manual test included in this package (segregation of duties)');
      }
    }
    ...
    if (decision === 'DENY') {
      throw new BadRequestException('Actor is not authorized to approve audit packages');
    }
    ...
    const [approval] = await this.prisma.$transaction([
      ...
      this.prisma.auditPackage.update({ where: { id: pkg.id }, data: { status: 'APPROVED', approved_by: approverId, approved_at: new Date() } }),
      this.prisma.outboxEvent.create({
        data: this.outbox.build({ tenantId, topic: CANONICAL_TOPICS.AUDIT_PACKAGE_APPROVED, eventType: 'audit_package.approved', payload: { packageId: pkg.id, manifestCoreHash: manifest.manifest_core_hash } }),
      }),
    ]);

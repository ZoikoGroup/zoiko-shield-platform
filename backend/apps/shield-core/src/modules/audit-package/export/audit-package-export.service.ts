...
export class AuditPackageExportService {
  ...
  async exportManifest(tenantId: string, packageId: string): Promise<Record<string, unknown>> {
    ...
    if (!manifest) {
      throw new NotFoundException(`AuditPackage '${packageId}' has no manifest`);
    }
  ...
  async exportEvidenceIndexJsonl(tenantId: string, packageId: string): Promise<string> {
    const manifest = await this.exportManifest(tenantId, packageId);
    const evidenceIndex = (manifest.evidenceIndex ?? []) as Record<string, unknown>[];
   ...
   */
  async exportToDirectory(tenantId: string, packageId: string, dirPath: string, includeEvidenceBytes = true): Promise<void> {
    const pkg = await this.prisma.auditPackage.findFirst({ where: { id: packageId, tenant_id: tenantId } });
    if (!pkg) {
      throw new NotFoundException(`AuditPackage '${packageId}' not found`);
    }
    if (pkg.status !== 'FROZEN') {
      throw new ConflictException(`AuditPackage '${packageId}' must be FROZEN to export for independent verification (currently ${pkg.status})`);
    }
    const manifest = await this.prisma.auditPackageManifest.findUnique({ where: { package_id: pkg.id } });
    if (!manifest || !manifest.manifest_content || !manifest.package_envelope_hash) {
      throw new NotFoundException(`AuditPackage '${packageId}' has no frozen manifest`);
    }

    mkdirSync(dirPath, { recursive: true });
    writeFileSync(join(dirPath, 'manifest.json'), manifest.manifest_content, 'utf-8');
    writeFileSync(
      join(dirPath, 'envelope.json'),
      JSON.stringify({ packageId: pkg.id, packageVersion: pkg.version, packageEnvelopeHash: manifest.package_envelope_hash }, null, 2),
      'utf-8',
    );

    if (includeEvidenceBytes) {
      const finalManifest = JSON.parse(manifest.manifest_content);
      const evidenceIndex: Array<{ evidenceId: string; vaultReference?: string }> = finalManifest.evidenceIndex ?? [];
      if (evidenceIndex.length > 0) {
        ...
        for (const entry of evidenceIndex) {
          ...
          try {
            const bytes = await this.storageService.getObject(entry.vaultReference);

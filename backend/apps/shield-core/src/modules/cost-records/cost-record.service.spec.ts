...
describe('CostRecordService (Part 26: internal economics, never customer-facing)', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [CostRecordService, { provide: PrismaService, useValue: prismaMock }],
    }).compile();
  ...
  it('computes total_cost as quantity * unit_cost', async () => {
    prismaMock.costRecord.create.mockImplementation(({ data }: any) => Promise.resolve(data));
  ...
  it('sums total cost across records for a usage class', async () => {
    prismaMock.costRecord.findMany.mockResolvedValue([{ total_cost: 10 }, { total_cost: 25 }]);

    const total = await service.getTotalCostByUsageClass('AI', new Date(), new Date());

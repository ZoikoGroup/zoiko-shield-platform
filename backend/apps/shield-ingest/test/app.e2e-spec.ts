...
describe('ShieldIngest Application Endpoints (e2e)', () => {
  ...
  beforeAll(async () => {
    prismaMock = {
      connectorInstance: {
        ...
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'conn-1', name: 'Entra Connector' }]),
        create: jest
          .fn()
          .mockResolvedValue({ id: 'conn-1', name: 'New Connector' }),
      ...
      normalizedEvent: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'norm-1', event_class: 'AUTHENTICATION' }]),
        findUnique: jest.fn().mockResolvedValue({
          id: 'norm-1',
          tenant_id: 'tenant-1',
      ...
      asset: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'asset-1', name: 'Host 1' }]),
        findFirst: jest.fn().mockResolvedValue(null),
      ...
      identityEntity: {
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'id-1', email: 'user@example.com' }]),
        findFirst: jest.fn().mockResolvedValue(null),
        create: jest
          .fn()
          .mockResolvedValue({ id: 'id-1', email: 'user@example.com' }),
      ...
      detectionRule: {
        ...
        findMany: jest
          .fn()
          .mockResolvedValue([
            { id: 'rule-1', name: 'Rule 1', status: 'ACTIVE' },
          ]),
        findUnique: jest.fn().mockResolvedValue({
          id: 'rule-1',
          name: 'Rule 1',
      ...
      alert: {
        ...
        update: jest
          .fn()
          .mockResolvedValue({ id: 'alert-1', status: 'ACKNOWLEDGED' }),
        count: jest.fn().mockResolvedValue(1),
      },
      resourceObservation: {
        findFirst: jest.fn().mockResolvedValue(null),
        create: jest
          .fn()
          .mockResolvedValue({ id: 'res-1', coverage_state: 'DISCOVERED' }),
        update: jest.fn().mockResolvedValue({ id: 'res-1' }),
        findMany: jest
          .fn()
          .mockResolvedValue([
            { id: 'res-1', canonical_resource_id: 'host-1' },
          ]),
      },
      usageRecord: {
        create: jest.fn().mockResolvedValue({
        ...
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'use-1', accepted_quantity: 1 }]),
      },
      case: {
        create: jest
          .fn()
          .mockResolvedValue({ id: 'case-1', title: 'Case 1', status: 'NEW' }),
        findUnique: jest.fn().mockResolvedValue({
          id: 'case-1',
          tenant_id: 'tenant-1',
          title: 'Case 1',
        ...
        findMany: jest
          .fn()
          .mockResolvedValue([
            { id: 'case-1', title: 'Case 1', status: 'NEW', severity: 'HIGH' },
          ]),
        update: jest
          .fn()
          .mockResolvedValue({ id: 'case-1', status: 'TRIAGED' }),
        count: jest.fn().mockResolvedValue(1),
      },
      caseTimeline: {
        create: jest
          .fn()
          .mockResolvedValue({ id: 'tl-1', event_type: 'CREATED' }),
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'tl-1', event_type: 'CREATED' }]),
      },
      caseDecision: {
        create: jest
          .fn()
          .mockResolvedValue({ id: 'dec-1', decision_type: 'TRIAGE_DECISION' }),
        findMany: jest
          .fn()
          .mockResolvedValue([
            { id: 'dec-1', decision_type: 'TRIAGE_DECISION' },
          ]),
      },
      evidenceRecord: {
        create: jest.fn().mockResolvedValue({
          ...
          raw_content: 'sample log content',
        }),
        findUnique: jest.fn().mockResolvedValue({
          id: 'ev-1',
          sha256_hash:
            'b14a7b8059d9c055954c92674ce6003202de35aa79e0a4fc018a7c265d361d42',
          raw_content: 'sample log content',
        }),
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'ev-1', sha256_hash: 'abc123hash' }]),
      },
      controlObjective: {
        create: jest
          .fn()
          .mockResolvedValue({ id: 'ctrl-1', code: 'MFA_ENFORCED' }),
        upsert: jest
          .fn()
          .mockResolvedValue({ id: 'ctrl-1', code: 'MFA_ENFORCED' }),
        findMany: jest
      ...
      controlTestRun: {
        ...
        findMany: jest
          .fn()
          .mockResolvedValue([{ id: 'run-1', result: 'PASS' }]),
      },
      assuranceReview: {
        create: jest.fn().mockResolvedValue({
          id: 'rev-1',
          overall_score: 100.0,
          period_name: '2026-Q3 Review',
        }),
        findMany: jest.fn().mockResolvedValue([
          {
            id: 'rev-1',
      ...
      claimEvaluation: {
        create: jest.fn().mockResolvedValue({
          id: 'eval-1',
          status: 'QUALIFIED',
          claim_key: 'CLAIM_15MIN_RESPONSE',
        }),
        findMany: jest.fn().mockResolvedValue([
          {
            id: 'eval-1',
  ...
  it('20. GET /api/v1/cases/case-1/decisions (List human decisions for case)', () => {
    return request(app.getHttpServer())
      .get('/api/v1/cases/case-1/decisions')
      .expect(200);
  });
  ...
  it('32. POST /api/v1/controls/objectives/ctrl-1/evaluate (Run automated control test)', () => {
    return request(app.getHttpServer())
      .post('/api/v1/controls/objectives/ctrl-1/evaluate')
      .expect(200);
  });
  ...
  it('37. POST /api/v1/assurance/reflections (Create vCISO strategic reflection)', () => {
    return request(app.getHttpServer())
      ...
      .send({
        category: 'STRATEGIC_RISK',
        title: 'MFA Roadmap',
        notes: 'Enforce MFA across admins',
      })
      .expect(201);
  });

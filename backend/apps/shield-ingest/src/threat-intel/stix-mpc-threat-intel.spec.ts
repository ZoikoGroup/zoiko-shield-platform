...
describe('StixThreatIntelMatcherService & MpcThreatMatcherService (LAB 20 Threat Intel & MPC PSI)', () => {
  ...
  beforeEach(async () => {
    ...
    stixMatcher = module.get<StixThreatIntelMatcherService>(StixThreatIntelMatcherService);
      ...
      const mockBundle: StixBundle = {
        ...
        objects: [
          ...
          },
          {
            ...
            confidence: 95,
            external_references: [{ source_name: 'MITRE ATT&CK', external_id: 'T1071.001' }],
          },
          {
            type: 'indicator',
            ...
            confidence: 90,
            external_references: [{ source_name: 'MITRE ATT&CK', external_id: 'T1566.002' }],
          },
        ],
      };
  ...
  describe('Privacy-Preserving MPC Private Set Intersection (PSI)', () => {
    it('should compute zero-knowledge matches against blinded tenant indicators without plaintext leakage', () => {
      ...
      const blindedIp = mpcMatcher.blindIndicator('198.51.100.99', tenantSecretKey);
      const blindedCleanIp = mpcMatcher.blindIndicator('10.20.30.40', tenantSecretKey);
      ...
      expect(mpcResult.matches[0].threatActorCampaign).toBe('APT29_CozyBear_C2');

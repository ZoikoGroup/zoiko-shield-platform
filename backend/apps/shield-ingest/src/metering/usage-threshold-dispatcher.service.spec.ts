...
describe('UsageThresholdDispatcherService (ZS-COM-BILL-001 MET-03 Threshold & Surge Safety)', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<UsageThresholdDispatcherService>(UsageThresholdDispatcherService);
  });

  it('dispatches 75% threshold warning when volume crosses 75% of quota', () => {
    const event = service.evaluateUsageThreshold('tenant-001', 'TELEMETRY_GB', 760, 1000);
  ...
  it('dispatches progressive 90% threshold warning when volume increases', () => {
    ...
    // 90% next
    const event90 = service.evaluateUsageThreshold('tenant-001', 'TELEMETRY_GB', 920, 1000);
  ...
  it('dispatches 100% threshold with SURGE_SAFE_CONTINUATION to prevent dropping critical logs', () => {
    const event100 = service.evaluateUsageThreshold('tenant-002', 'TELEMETRY_GB', 1050, 1000);
  ...
  it('deduplicates alerts so the same tier is not dispatched multiple times in the same window', () => {
    const firstCall = service.evaluateUsageThreshold('tenant-003', 'TELEMETRY_GB', 780, 1000);
    expect(firstCall).not.toBeNull();

    const secondCall = service.evaluateUsageThreshold('tenant-003', 'TELEMETRY_GB', 790, 1000);

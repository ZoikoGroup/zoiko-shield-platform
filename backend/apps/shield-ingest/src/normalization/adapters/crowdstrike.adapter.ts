...
export class CrowdStrikeOcsfAdapter {
  static normalize(payload: Record<string, any>): NormalizedOcsfEvent {
    const isTerminate =
      payload.eventSimpleName === 'ProcessTerminate' ||
      payload.activity === 'TERMINATE';
    const isSuspicious =
      (payload.CommandLine &&
        (payload.CommandLine.includes('mimikatz') ||
          payload.CommandLine.includes('powershell -enc'))) ||
      payload.severity === 'CRITICAL' ||
      payload.severity === 'HIGH';

    const actorUserId =
      payload.UserName || payload.actorUserId || payload.user?.id || 'system';
    const actorEmail =
      payload.UserEmail ||
      payload.actorEmail ||
      `${actorUserId}@endpoint.local`;

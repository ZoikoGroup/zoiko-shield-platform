...
describe('EntraTokenService', () => {
  ...
  beforeEach(async () => {
    credentialMock = { resolveClientSecret: jest.fn().mockResolvedValue('super-secret') };
    process.env.ENTRA_CLIENT_ID = 'client-id-under-test';

    const module: TestingModule = await Test.createTestingModule({
      providers: [EntraTokenService, { provide: CredentialService, useValue: credentialMock }],
    }).compile();
  ...
  it('throws ConnectorAuthenticationError instead of returning a fake token when externalTenantId is missing', async () => {
    await expect(service.getAccessToken('instance-1', '')).rejects.toThrow(ConnectorAuthenticationError);
  });
  ...
  it('throws ConnectorAuthenticationError instead of silently succeeding when Microsoft rejects the token request', async () => {
    ...
    await expect(service.getAccessToken('instance-1', 'external-tenant-1')).rejects.toThrow(
  ...
  it('caches a successfully acquired token and does not re-fetch until near expiry', async () => {
    ...
    const first = await service.getAccessToken('instance-1', 'external-tenant-1');
    const second = await service.getAccessToken('instance-1', 'external-tenant-1');
  ...
  it('decodes the roles claim out of a JWT access token', () => {
    const payload = Buffer.from(JSON.stringify({ roles: ['User.Read.All', 'AuditLog.Read.All'] })).toString(
      'base64url',
    );
    const fakeJwt = `header.${payload}.signature`;

    expect(service.decodeGrantedRoles(fakeJwt)).toEqual(['User.Read.All', 'AuditLog.Read.All']);
  });

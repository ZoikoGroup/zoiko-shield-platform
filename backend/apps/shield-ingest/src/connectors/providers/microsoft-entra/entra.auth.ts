...
export class EntraAuthService {
  ...
  private readonly clientId = process.env.ENTRA_CLIENT_ID || '';
  ...
  generateAuthUrl(customerTenantId: string, state: string): string {
    if (
      !this.clientId ||
      (!process.env.ENTRA_REDIRECT_URI && process.env.NODE_ENV === 'production')
    ) {
      throw new Error(
        'ENTRA_CLIENT_ID and ENTRA_REDIRECT_URI must be configured',
      );
    }

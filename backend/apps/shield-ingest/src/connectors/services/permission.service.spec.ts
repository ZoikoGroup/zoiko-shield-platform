...
describe('PermissionService', () => {
  ...
  it('declares each required permission as ungranted until verified', async () => {
    await service.declareRequired('tenant-a', 'instance-1', 'microsoft-entra', ['User.Read.All', 'AuditLog.Read.All']);
    ...
    expect(prismaMock.connectorPermission.upsert).toHaveBeenCalledWith(
      expect.objectContaining({ create: expect.objectContaining({ permission: 'User.Read.All', granted: false }) }),
    );
  });
  ...
  it('flags previously-granted permissions absent from the fresh grant set as drift and publishes connector.permission.changed.v1', async () => {
    prismaMock.connectorPermission.findMany.mockResolvedValue([
      { id: 'p1', tenant_id: 'tenant-a', permission: 'User.Read.All', granted: true },
      { id: 'p2', tenant_id: 'tenant-a', permission: 'AuditLog.Read.All', granted: true },
    ]);

    const { newlyMissing } = await service.reconcileGranted('instance-1', ['User.Read.All']);
    ...
    expect(kafkaMock.publishEvent).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ tenantId: 'tenant-a', instanceId: 'instance-1', lostPermissions: ['AuditLog.Read.All'] }),
    );
  });

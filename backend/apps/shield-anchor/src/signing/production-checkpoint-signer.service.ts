...
export class ProductionCheckpointSigner implements CheckpointSigner {
  ...
  constructor() {
    ...
    this.algorithm = (process.env.ANCHOR_KMS_SIGNING_ALGORITHM ??
      'ECDSA_SHA_256') as SigningAlgorithmSpec;
    if (!this.keyId)
      throw new Error('ANCHOR_KMS_KEY_ID is required in production');
    if (!SUPPORTED_ALGORITHMS.has(this.algorithm)) {
      throw new Error(
        `Unsupported ANCHOR_KMS_SIGNING_ALGORITHM '${this.algorithm}'`,
      );
    }
  ...
  async sign(merkleRoot: string): Promise<CheckpointSignResult> {
    ...
    if (!outcome.Signature)
      throw new Error('AWS KMS returned no checkpoint signature');
    return {
    ...
    this.publicKeyPromise ??= this.client
      ...
      .then((outcome) => {
        if (!outcome.PublicKey)
          throw new Error('AWS KMS returned no checkpoint public key');

import { CosignBinaryAttestorService, ArtifactDigestMetadata } from './cosign-binary-attestor.service';
...
describe('CosignBinaryAttestorService (LAB 17/18 Supply Chain & Binary Authorization)', () => {
  ...
  const validMetadata: ArtifactDigestMetadata = {
    imageRepository: 'europe-west3-docker.pkg.dev/zoiko-prod/runtime/shield-core',
    ...
    cosignKmsKeyUri: 'gcp-kms://projects/zoiko-security/locations/europe-west3/keyRings/kr1/cryptoKeys/cosign-root',
  };
  ...
  it('should sign artifact and grant Binary Authorization admission for valid SLSA Level 3 image', () => {
    ...
    const admission = attestorService.evaluateAdmissionPolicy(validMetadata, signRes.signature, true);
  ...
  it('should deny admission when signature is invalid or unsigned', () => {
    ...
    const admission = attestorService.evaluateAdmissionPolicy(validMetadata, invalidSignature, true);
  ...
  it('should deny admission when KMS signer is untrusted', () => {
    const signRes = attestorService.signArtifactDigest(validMetadata);
    const admission = attestorService.evaluateAdmissionPolicy(validMetadata, signRes.signature, false);
    expect(admission.isAdmissionGranted).toBe(false);
  });
});

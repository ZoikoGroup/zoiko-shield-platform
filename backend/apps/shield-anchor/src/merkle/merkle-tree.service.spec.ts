...
describe('MerkleTreeService (ZS-MERKLE-V1)', () => {
  ...
  it('produces a valid inclusion proof for every leaf, single-leaf case', () => {
    ...
    expect(
      service.verifyInclusion('leaf-a', result.proofs[0], result.root),
    ).toBe(true);
  });
  ...
  it('produces valid inclusion proofs for every leaf, multi-leaf odd count (duplicate-last-node case)', () => {
    ...
    leaves.forEach((leaf, i) => {
      expect(service.verifyInclusion(leaf, result.proofs[i], result.root)).toBe(
        true,
      );
    });
  });
  ...
  it('a wrong sibling hash in the proof fails inclusion — never a false positive', () => {
    ...
    const tamperedProof = result.proofs[0].map((step) => ({
      ...step,
      siblingHash: '0'.repeat(64),
    }));
    expect(service.verifyInclusion('leaf-a', tamperedProof, result.root)).toBe(
      false,
    );
  });

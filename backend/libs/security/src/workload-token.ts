...
function signingMaterial(): { key: string; algorithm: Algorithm } {
  if (process.env.WORKLOAD_IDENTITY_PRIVATE_KEY_PEM) {
    return { key: process.env.WORKLOAD_IDENTITY_PRIVATE_KEY_PEM.replace(/\\n/g, '\n'), algorithm: 'RS256' };
  }
  if (process.env.NODE_ENV !== 'production' && process.env.WORKLOAD_IDENTITY_DEV_SECRET) {
    return { key: process.env.WORKLOAD_IDENTITY_DEV_SECRET, algorithm: 'HS256' };
  }
...
function verificationMaterial(): { key: string; algorithms: Algorithm[] } {
  if (process.env.WORKLOAD_IDENTITY_PUBLIC_KEY_PEM) {
    return { key: process.env.WORKLOAD_IDENTITY_PUBLIC_KEY_PEM.replace(/\\n/g, '\n'), algorithms: ['RS256'] };
  }
  if (process.env.NODE_ENV !== 'production' && process.env.WORKLOAD_IDENTITY_DEV_SECRET) {
    return { key: process.env.WORKLOAD_IDENTITY_DEV_SECRET, algorithms: ['HS256'] };
  }
...
export function createWorkloadToken(audience: string): string {
  ...
  if (!subject) throw new Error('SERVICE_NAME is required to issue a workload token');
...
export function verifyWorkloadToken(token: string, audience: string): JwtPayload {
  ...
  if (typeof result === 'string' || !result.sub || !result.jti || !result.exp || !result.iat) {
...
export function workloadAuthorizationHeaders(audience: string): Record<string, string> {
